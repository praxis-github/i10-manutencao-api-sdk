...
class ApiSdk
{
    ...
    public function __construct($baseUrl = 'http://gcpmail3.task.com.br/v1/')
    {
     ...
     * @return I10ManutencaoApiSdk
     */
    public function dispatchNotification(string $title, string $body, array $usersIds)
    {
        $endpoint = 'devices/notify';

        try {
            $this->client->post(
                $endpoint,
                [
                    'body' => json_encode(['title' => $title, 'body' => $body, 'usersIds' => $usersIds]),
                    'headers' => [
                        'Content-Type' => 'application/json'
                    ]
                ]
            ...
            if ($statusCode === 401) {
                $this->refreshToken($this->accessToken);
                return $this->dispatchNotification($title, $body, $usersIds);
            }
            throw new \Exception(sprintf('Error : %s', $e->getMessage()));
        }
        return $this;
